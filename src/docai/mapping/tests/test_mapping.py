...
from docai.database.models import Page as ORMPage
from docai.database.models import Query as ORMQuery

# Import DTO models (assumed in docai.database.schemas)
from docai.database.schemas import Document as DTODocument
from docai.database.schemas import MinimalDocument as DTOMinimalDocument
from docai.database.schemas import MinimalQuery as DTOMinimalQuery
from docai.database.schemas import Page as DTOPage
from docai.database.schemas import Query as DTOQuery
...
from docai.shared.models.document import MinimalDocument as DomainMinimalDocument
from docai.shared.models.document import Page as DomainPage
from docai.shared.models.query import MinimalQuery as DomainMinimalQuery
from docai.shared.models.query import Query as DomainQuery


# Sample synthetic data for testing
def create_sample_domain_page():
    return DomainPage(id="page_001", page_number=1, image_path="/fake/path/page1.jpg")
...
class TestDomainDTOConversions(unittest.TestCase):
    ...
    def test_domain_to_dto_minimal_document(self):
        ...
        minimal_domain_doc = DomainMinimalDocument(
            ...
            updated_at=domain_doc.indexed_at
            or domain_doc.processed_at
            or domain_doc.created_at,
        )
    ...
    def test_domain_to_dto_query(self):
        ...
        self.assertEqual(
            dto_query.target_document_ids, domain_query.target_document_ids
        )
    ...
    def test_dto_to_domain_page(self):
        ...
        dto_page = DTOPage(
            id="dto_page_001", page_number=1, image_path="/fake/path/page1.jpg"
        )
        domain_page = domain_dto.dto_to_domain_page(dto_page)
        self.assertEqual(
            domain_page.id if hasattr(domain_page, "id") else domain_page.page_id,
            dto_page.id,
        )
    ...
    def test_dto_to_domain_document(self):
        ...
        self.assertEqual(
            domain_doc.id if hasattr(domain_doc, "id") else domain_doc.doc_id,
            dto_doc.id,
        )
    ...
    def test_dto_to_domain_query(self):
        ...
        self.assertEqual(
            domain_query.id if hasattr(domain_query, "id") else domain_query.query_id,
            dto_query.id,
        )
        self.assertEqual(domain_query.text, dto_query.text)
        self.assertEqual(
            domain_query.target_document_ids, dto_query.target_document_ids
        )
...
class TestUtilsLinkDocumentsToQuery(unittest.TestCase):
    ...
    def test_link_documents_to_query_success(self):
        ...
        updated_query = utils.link_documents_to_query(
            self.db_service, self.orm_query, ["doc_001"]
        )
    ...
    def test_link_documents_to_query_failure(self):
        ...
        self.db_service.get_documents_by_ids.side_effect = ValueError(
            "Document not found"
        )
        with self.assertRaises(ValueError):
            utils.link_documents_to_query(
                self.db_service, self.orm_query, ["doc_missing"]
            )
